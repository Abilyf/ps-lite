    ...
    bool *end_preplugin)
{
    ...
    char *token = strsep(&line, " ");//作用是分离选中的字符，返回左边的字符串，同时将line修改为右边字符串
    if (token == NULL) {
...
int plugin_parse_plugin_id(const char *plugin_fname, char *plugin_id, bool *require_negotiation) {
    FILE *file = fopen(plugin_fname, "r");//打开plugin指向的.PLUGIN文件
    ...
    size_t len = 0;
    ssize_t read = getline(&first_line, &len, file);//getline没有指定结束符时，默认以'\n'为结束标志，也即这里只是读取第一行数据
    if (read == -1) {
        printf("Error in the file %s\n", plugin_fname);
        fclose(file);
        return 1;
...
protoop_arg_t plugin_run_protoop_internal(picoquic_cnx_t *cnx, const protoop_params_t *pp) {
    if (pp->inputc > PROTOOPARGS_MAX) {//当输入的可变参数数目大于最大值时
    ...
    cnx->protoop_inputv = pp->inputv;//将此前输入的可变参数数组赋值给一并传入的cnx中
    cnx->protoop_inputc = pp->inputc;//将此前输入的可变参数数目赋值给一并传入的cnx中
    cnx->protoop_outputv = pp->outputv;//将此前的输出可变参数数组赋值给一并传入的cnx中
    ...
    protocol_operation_struct_t *post;
    if (pp->pid->hash == 0) {
        pp->pid->hash = hash_value_str(pp->pid->id);//设定pid的哈希值
    }
