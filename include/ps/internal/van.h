...
namespace ps {
    ...
    public:
        ...
        virtual void NewTracker(Key init_key, int tensor_size, const Callback& cb = nullptr) { };

        virtual void WriteBack(const SArray<Key>& key, SArray<char>& val) { };
        virtual void SetPriorityNum(int num) { };

        //virtual int QUICConnectRecv(char node_id) {};
        /**
         ...
         */
         ...
         //not const
        int Send(Message& msg, Node* node, int connect);
    ...
    protected:
         ...
         */
         ...
         //not const
        virtual int SendMsg(Message& msg, Node* node, bool connect) = 0;

        //update node id
        virtual void UpdateNodeID(int old_id, int new_id) = 0;

        /**
       * \brief build a connection
       * \return return 1 if succeed, or -1 if failed
       */
       //virtual int QUICCnxRecv() = 0;
    ...
    private:
        ...
        /** thread function for connection building*/
        //void CnxBuilding();
        ...
        // 表明add_node阶段worker或server节点是否已完成节点间的相互连接
        std::unordered_map<int, int> add_node_record_;
        ...
        /** the thread for connection building */
        std::unique_ptr<std::thread> cnxBuild_thread_;
