...
namespace ps {
    ...
    struct Control {
        ...
        /** \brief all commands */
        enum Command { EMPTY, TERMINATE, ADD_NODE, BARRIER, ACK, HEARTBEAT, CONNECT };
        /** \brief the command */
        Command cmd;
        /** \brief node infos */
    ...
    struct Meta {
        ...
        int recver;
        /** \brief whether or not this msg has data*/
        int more;
    ...
    struct Message {
        ...
        void AddData(const SArray<V>& val) {
            ...
            meta.data_type.push_back(GetDataType<V>());//这一步获取模板参数的类型，将其推入meta中的data_type中，用来依次存储存入msg->data中的数据类型
            SArray<char> bytes(val);//建立一个bytes数组，将传入的数据转为char类型
            meta.data_size += bytes.size();//将传入的数据的大小更新到meta中的data_size中
            data.push_back(bytes);//将数据推入msg->data
        }
        std::string DebugString() const {
            std::stringstream ss;
            ss << meta.DebugString();
